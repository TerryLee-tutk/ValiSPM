...
3.2.0.0     |Terry Liu         |2019-04-12     |+ Add avSendJSONCtrl for JSON IO control
3.2.0.0     |Terry Liu         |2019-04-12     |+ Add avServNotifyVSaasPullStream, avServGetVSaasContractInfo, avEnableVSaaS for VSaaS 4.0
3.3.0.0     |Terry Liu         |2019-05-01     |+ Add avSendJSONCtrlRequest, avFreeJSONCtrlResponse
3.3.0.0     |Terry Liu         |2019-05-01     |o Mark avSendJSONCtrl ad deprecated
4.0.0.0     |Terry Liu         |2019-08-07     |+ Add avEnableVSaaSByNebula
4.0.0.37    |Ewan Chen         |2019-11-25     |o Mark AV_Set_Log_Path as deprecated
 */
...
typedef struct VSaaSContractInfo {
    ...
    unsigned int event_recording_max_sec;
    unsigned int video_max_fps;
    unsigned int recording_max_kbps;
    unsigned int video_max_high;
...
typedef int(__stdcall *avSendJSONCtrlResponse)(int av_index, const char *json);

typedef int(__stdcall *avRecvJSONCtrlRequestFn)(int av_index, const char *func, const NebulaJsonObject *json_args_obj, NebulaJsonObject **json_response_obj); 
...
/**
 * \details The information of the pulling stream.
 *
 * \param pullstream_type     [in] The type of event trigger. ex : movement, temperature... (general value is 999).
 * \param starttime           [in] The start timestamp of the record video.
 *                                 This value can be "live" which means to get live video from camera directly.
*/

typedef struct VSaaSPullStreamAttr {
    VSaaSPullStreamType pullstream_type;
    char starttime[MAX_TIMESTAMP_LENGTH + 1];
    char filename[MAX_PLAYBACK_FILENAME_LENGTH + 1];
} VSaaSPullStreamAttr;
 ...
 *
 */
AVAPI_API int avEnableVSaaS(const char *udid, const char *VSaaSconfig, OnVSaaSConfigChangedCb config_callback, OnVSaaSUpdateContractInfoCb contract_callback);
 ...
 *
 */
AVAPI_API int avEnableVSaaSByNebula(NebulaDeviceCtx *device_ctx, const char *config, OnVSaaSConfigChangedCb config_callback, OnVSaaSUpdateContractInfoCb contract_callback);

/**
 * \brief Get VSaaS contract information from VSaaS server.
 *
 * \details This function is used by device to get contract information from VSaaS server.
 *          This function must be called after avEnableVSaaS() or avEnableVSaaSByNebula().
 *          
 * \param contractInfo [out] The VSaaS contract information and video format that requested by VSaaS server.
 * \param timeout_sec [in] timeout of this function.
 *                      
 * \return #AV_ER_NoERROR if notify VSaas server successfully.
 * \return Error code if return value < 0
 *          - #AV_ER_VSAAS_PULLING_NOT_ENABLE not enable VSaaS pulling stream.
 *          - #AV_ER_FAIL_CONNECT_TO_VSAAS  get error from https while connect to VSaaS server.
 *
 * \see avEnableVSaaS()
 *
 */
...
AVAPI_API int avServGetVSaasContractInfo(VSaaSContractInfo* contractInfo, unsigned int timeout_sec);

/**
 * \brief Sending event notification to VSaaS server and asking server to pull streaming
 *
 * \details This function is used by device to notify VSaaS server of pulling stream by AVAPI.
 *          This function must be called after avEnableVSaaS() or avEnableVSaaSByNebula().
 *
 * \param attr [in] the information of event which including event type and timestamp.
 * \param timeout_sec [in] timeout of this function.
 *                      
 * \return #AV_ER_NoERROR if notify VSaas server successfully.
 * \return Error code if return value < 0
 ...
 *
 */
AVAPI_API int avServNotifyVSaasPullStream(const VSaaSPullStreamAttr *attr, unsigned int timeout_sec);
 ...
 * \param json [in] The json data that need to send to AV server.
 * \param timeout_sec [in] The timeout for this function in unit of second, blocks forever if 0.
 *
 * \return #AV_ER_NoERROR if sending successfully
 * \return Error code if return value < 0
 ...
 * \attention (1) This API is a blocking function and it will wait until
 *              having acknowledgment from the receiver.
 *            (2) This API has been deprecated please replace it with avSendJSONCtrlRequest
 */
AVAPI_API_DEPRECATED int avSendJSONCtrl(int av_index, const char *json, unsigned int timeout_sec);
...
* \param json_response [out] The pointer of av response, please do not allocate memory of this parameter
* \param timeout_sec [in] The timeout for this function in unit of second, blocks forever if 0.
*
* \return #AV_ER_NoERROR if sending successfully
* \return Error code if return value < 0
...
*/
AVAPI_API int avSendJSONCtrlRequest(int av_index, const char *json_request, NebulaJsonObject **josn_response_obj, unsigned int timeout_sec);
...
*
* \param json_response [in] This json response data pointer
*
* \return #AV_ER_NoERROR if sending successfully
* \return Error code if return value < 0
*            - #AV_ER_INVALID_ARG The AV channel ID is not valid
...
* \attention This function is not support for av server now.
*/
AVAPI_API int avFreeJSONCtrlResponse(NebulaJsonObject *josn_response_obj);
