...
4.0.0.0     |Terry Liu         |2019-08-07     |- Remove IOTC_Set_Master_Region
4.0.0.0     |Terry Liu         |2019-08-07     |+ Add IOTC_Device_Login_By_Nebula, IOTC_Device_Listen_By_Nebula, IOTC_Client_Connect_By_Nebula
4.0.0.37    |Ewan Chen         |2019-11-15     |o Mark IOTC_Set_Log_Path as deprecated
 */
...
struct st_SInfoEx
{
	...
	unsigned char isNebula; //!< 0: Session not created by nebula, 1: Session created by nebula
};
...
/**
 * \brief Used by a device to stop login to IOTC servers
 *
 * \details This function stop the login process of IOTC device and release all the connections to
 *          IOTC servers and free the socket resource. After the call of IOTC_Device_Logout(),
 *          the device can not be connect by any client.
 *
 * \return #IOTC_ER_NoERROR if login process stop successfully
 * \return Error code if return value < 0
 *			- #IOTC_ER_NOT_INITIALIZED The IOTC module is not initialized yet
 *			- #IOTC_ER_SESSION_IN_USE There is at least one IOTC session in use
 *
 * \see IOTC_Device_Login(), IOTC_Device_LoginEx()
 *
 * \attention (1) Please disconnect all the IOTC sessions before call this function 
 *            (2) Please use IOTC_Device_Login() or IOTC_Device_LoginEx() to 
 *                re-login to IOTC servers.
 */
P2PAPI_API_DEPRECATED int IOTC_Device_Logout(void);
